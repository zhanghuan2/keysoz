...
const itemCompareTemplate = Handlebars.templates["buyer/goods_search/templates/item-compare"]
const searchTemplate = Handlebars.templates["buyer/goods_search/templates/search"]
const categoryListTemplate = Handlebars.templates["buyer/goods_search/templates/category-list"]
const goodsListTemplate = Handlebars.templates["buyer/goods_search/templates/goods-list"]
const addToCartForm = Handlebars.templates["buyer/goods_search/templates/add-to-cart-form"]
const otherAttrSearch = Handlebars.templates["buyer/goods_search/templates/other-attr-search"]
const addressSelectDown = Handlebars.templates["buyer/goods_search/templates/address-select-down"]
...
class GoodsList extends AddressGroup {
  ...
  // 优化浮动搜索栏的布局： 显示更多服务筛选
  optimizeFixedBarLayout(serviceProtocols){
    this.servFilterLayoutControl(serviceProtocols, 'fix-nav-bar');
  }
  ...
  bindFixedBarEvent(){
    ...
    $('.fix-nav-bar .js-24Hours').on('click', (evt)=> {    // 24小时送达服务
      $('.goods-search-list .js-24Hours').trigger('click');
    });
    $('.fix-nav-bar .js-48Hours').on('click', (evt)=> {    // 48小时送达服务
      $('.goods-search-list .js-48Hours').trigger('click');
    });
  }
  ...
  getInitData(){
    ...
    $.ajax({
      ...
      $('.deliveryAddress').empty().append(addressSelectDown(data.addr));
      self.showServiceFilter(data.serviceProtocols);
      self.optimizeFixedBarLayout(data.serviceProtocols);
      $('.total span').text(_DATA_.entities.total);
      $('.custom-total').text( Math.floor(_DATA_.entities.total / self.pageSize ) + 1);
  ...
  // 控制服务筛选的布局
  servFilterLayoutControl(serviceProtocols, container){
    let expandServiceNum = 6;
    let limit = expandServiceNum - 4;
    if(serviceProtocols.length > limit){
      $('.'+ container + ' .filter-checkbox .hasService').slice(limit, serviceProtocols.length).each(function(){
        $(this).remove();
      })
      $('.'+ container +' .filter-checkbox .hasMoreService').slice(0, limit).each(function(){
        $(this).remove();
      });
      $('.'+ container +' .show-more-filter').show();
      
      $('.'+ container +' .filter-container').addClass('shop-filter-box');
      $('.'+ container +' .show-more-filter').on('mouseenter', (evt)=>{
        let target = $(evt.currentTarget);
        ...
        $('.'+ container +' .moreFilter').css('display', 'inline-block');
      })
      $('.'+ container +' .shop-filter-box').on('mouseleave', (evt)=>{
        let target = $(evt.currentTarget);
  ...
  showServiceFilter(serviceProtocols) {
    if(serviceProtocols){
      for(let i=0; i<serviceProtocols.length; i++){
        ...
        $('.filter-checkbox input[name="'+ servName +'"]').data('service-id', servId).closest('.box').show().addClass('hasService');
        $('.filter-checkbox .moreFilter input[name="'+ servName +'"]').data('service-id', servId).closest('.box').show().removeClass('hasService').addClass('hasMoreService');
      }

      // 绑定服务筛选事件：上传票证服务，送货上门服务，上门安装服务
      $('.goods-search-list .js-ticketServ, .goods-search-list .js-deliveryServ, .goods-search-list .js-installServ, .goods-search-list .js-24Hours, .goods-search-list .js-48Hours').on('change', (evt) => this.serviceFilter(evt));
      // 显示更多按钮
      if(this.isShop){
        // 店铺页均默认展示6个，超过6个点击更多进行展示筛选
        this.servFilterLayoutControl(serviceProtocols, 'goods-search-list');
      }
      else {
  ...
  processCheckedService(servNameArr){
    let servArr = [];
    for(let i=0; i<servNameArr.length; i++){
      let servEle = $('.goods-search-list .js-'+servNameArr[i]);
      let servState = servEle.prop('checked')?1:0;
      if(servState){
        servArr.push( servEle.data('service-id') );
        $('.fix-nav-bar .js-'+servNameArr[i]).prop('checked', true);
      }else {
        $('.fix-nav-bar .js-'+servNameArr[i]).prop('checked', false);
      }
    }
    return servArr;
  }
  ...
  serviceFilter(evt) {
    let self = this;
    let servNameArr = ['ticketServ', 'deliveryServ', 'installServ', '24Hours', '48Hours'];
    ...
    servArr = this.processCheckedService(servNameArr);
