...
class OrderFromCreateByRequisition {
  ...
  preRender() {
    this.$orderCreatePage = $('.order-form-create-by-requisition')
    this.$itemsPlans = $('.items-plans-list')
    this.$additionInfo = $('.order-addition-info')
    this.$itemsList = $('.requisition-item-list')
    ...
    this.renderRequisitionItemsList()//渲染需求商品列表金额总计
  }
  bindEvents() {
  ...
  bindPurchasePlanModalEvent(templates, modal){
    ...
    $(modal.modal).delegate('.js-new-payment-items-submit', 'click', (evt)=>{
      ...
      if (planId) {
        self.showPurchasePlanItemsModal(planId, planInfo, modal)
      } else {
        new Modal({
          title:"提示",
          icon:"warning",
  ...
  modifyPlanBindItem(evt) {
    ...
    planItemsModal.show()
    renderItemsCount(planItemsModal.modal)
    //提交事件
    ...
    //计算可操作的绑定数量
    function renderItemsCount(modal){
  ...
  showPurchasePlanItemsModal(confirmationId, planInfo, preModal){
    ...
    $.ajax({
      ...
      planItemsModal.show()
      renderItemsCount(planItemsModal.modal)
      autoFillRequisitionItems(planItemsModal.modal)
      bindModalEvent(planItemsModal)
    }).fail(()=>{
      $('.js-new-payment-items-submit').prop('disabled', false)
    })
    ...
    //关联采购计划时自动勾选需求商品
    function autoFillRequisitionItems(modal){
    ...
    function  bindModalEvent(planItemsModal) {
  ...
  submitOrderData(evt, needContract) {
    let orderData = {
      ...
      skuMap: {},
      commentForSupplier: {},
      requisitionIds: this.orderItems.requisitionIds
    }
    //收货地址
    let address = this.$orderCreatePage.find('.address-block.active').data('address')
    if(!address){
    ...
    this.$itemsList.find('input.remark-input').each((i, input)=>{
      ...
      if (sellerId) {
        ...
        $(input).closest('.requisition-items-table').find('.js-seller-id-input').each((i,el) => {
          sellerId = $(el).val()
          orderData.commentForSupplier[sellerId] = $(input).val()
        })
      }
    ...
    $(evt.target).prop('disabled', true)
    $.ajax({
      url: '/api/zcy/orders/createFromRequisitions',
      type: 'post',
      contentType: 'application/json',
  ...
  bodyScroll() {
    let left = 196 - $(window).scrollLeft()
    this.$additionInfo.css('left', left)
  }
}

module.exports = OrderFromCreateByRequisition