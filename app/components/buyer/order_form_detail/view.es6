...
class OrderFormDetail {
  constructor($) {
    ...
    //发货/补发货
    //this.shipmentCreate = $('.js-create-shipment');
    ...
    this.listSwitchBtn = $('.list-switch').find('span');//切换列表
    this.bindEvent();
    this.moreHandle();
  ...
  bindEvent() {
    ...
    //发货/补发货
    //this.shipmentCreate.on('click', (evt) => this.createShipment(evt));
    ...
    this.listSwitchBtn.on('click',(evt) => this.listSwitch(evt));
  }
  ...
  //渲染关联需求单字段
  renderSourceIds(){
    let linksDiv = $('.requisition-id-links'),
        sourceIds = linksDiv.data('ids')
    if(!sourceIds) return
    linksDiv.empty()
    ...
    _.each(ids, (id)=>{
      if(!html){
        html = '<a href="/buyer/requisitions/detail?reqId='+id+'">'+id+'</a>'
      }
      else{
        html = html + ',&nbsp;<a href="/buyer/requisitions/detail?reqId='+id+'">'+id+'</a>'
      }
    })
    linksDiv.html(html)
  }
  ...
  changeConfirmModal(evt) {
    ...
    $.ajax({
      ...
      data: `orderId=${orderId}`,
      success: (data) => {
        let changeConfrimBookModal = new Modal(changeConfrimBookTemplates({
          data: data
        }));
  ...
  //列表切换
  listSwitch(evt){
    if($(evt.target).hasClass('goodsList')){
      if($(evt.target).hasClass('list-active')){
        return;
      }else{
        $(evt.target).addClass('list-active');
        $(evt.target).siblings('span').removeClass('list-active');
        $('.goodsListContent').css('display','block');
        $('.requireListContent').css('display','none');
      }
    }else if($(evt.target).hasClass('requireList')){
      if($(evt.target).hasClass('list-active')){
        return;
      }else{
        $(evt.target).addClass('list-active');
        $(evt.target).siblings('span').removeClass('list-active');
        $('.requireListContent').css('display','block');
        $('.goodsListContent').css('display','none');
      }
    }
  }
