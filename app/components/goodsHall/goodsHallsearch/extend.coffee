...
Query = require "common/query/extend"
Language = require "locale/locale"
itemCompareTemplates = Handlebars.templates["goodsHall/goodsHallsearch/templates/item-compare"]
...
class ItemsList
  constructor: ($) ->
    ...
    @$hasSupplierBtn = $("[name=filterByDealer]")
    @bindEvent()
    @initSaleFt()
    @init()
    #排序
  ...
  bindEvent: ->
    ...
    @$hasSupplierBtn.on "click", @hasSupplierSubmit
  ...
  #分页初始化
  $(".main-pagination")[0]&&new PaginationClass(".main-pagination").total($(".main-pagination").attr("data-total")).show(20,{num_display_entries: 5, jump_switch: true, maxPage: -1, page_size_switch: false});
  #列表切换
  list_stylechange: ->
    $(".list_style").removeClass("hide")
    $(".table_style").addClass("hide")
    window.location.search=$.query.set("pageSize",20).set("pageNo",1).set("showType","img")
  table_stylechange: ->
    $(".table_style").removeClass("hide")
    $(".list_style").addClass("hide")
    window.location.search=$.query.set("pageSize",10).set("pageNo",1).set("showType","list")
  #提交价格区间筛选
  ...
  #提交价格区间筛选
  hasSupplierSubmit: (evt)->
    initQuery = new Query();
    window.location.search = initQuery.set("filterByDealer",$("[name=filterByDealer]").prop("checked")).set("pageNo",1).toString()
