...
class reverseDistrict {
  ...
  bindEvent() {
    ...
    $('.icon-sousuo-').on('click', function() {
      window.location.search = $.query.remove('pageNo').remove('pageSize').remove('error').set('code', vm.$inputPurchase.val())
    })
    ...
    this.$searchModal.find('.selectric').on('click', function() {
      ...
      $('.location-container').slideToggle()
        // if ($(':radio[name=reverse-location]:checked').val() == '1') {
        // }
    })
    ...
    // 高级搜索 点击确定
    $('.btn-reverse-sure').on('click', function() {
      ...
      window.location.search = $.query.remove('pageNo').remove('pageSize').remove('error').set('code', vm.$inputKeyword.val()).set('startTime', $('input[name=startTime]').val()).set('endTime', $('input[name=endTime]').val()).set('districtCode', encodeURI(selectedCodes.join(','))).toString()

    })

    // 重置
    $('.btn-reverse-reset').on('click', function() {
      //关键字清空
      vm.$inputKeyword.val('')
        //外层输入框清空
      vm.$inputPurchase.val('')
      ...
      $('.radio-reverse-first').trigger('click')
      $('.district-text').html('请选择所在地')
        //重置询价时间
      ...
      let start = ''
      let end = ''
      $('.time-position').append(`<span class="container-date"><input class="date1 search date-input mr" name="startTime" value="${start}" readonly  type="text" placeholder="开始时间">-<input class="date1 search date-input mr ml" name="endTime" readonly value="${end}"  type="text" placeholder="结束时间"></span>`)
      $('.date1').datepicker()
    })
  ...
  // 市级 http://reversehall.zcy.gov.cn/api/district/getDistrictTree?deep=1&pid=953&_=1489652356190
  // 县级 http://reversehall.zcy.gov.cn/api/district/getDistrictTree?deep=1&pid=968&_=1489652356191
  // 市县两级 http://reversehall.zcy.gov.cn/api/district/getDistrictTree?deep=3&pid=953
...
export default reverseDistrict
