...
class Area
  ...
  exampleBindEvents: ->
    ...
    @$el.on "change", ".js-check-area", @checkArea
    @$el.on "click", ".js-save-template", @submitRegion

  getAreas: =>
    $.ajax
      url: "/api/address/unionRegions"
      type: "GET"
      success: (data) =>
        @unionRegions = _.map data, (i) =>
          i.regions = i.regionIds.split(",")
          i
      error: =>
  ...
  getRegion: (evt)=>
    li = $(evt.currentTarget).closest("li")
    provinceLi = $(evt.currentTarget).closest(".province-li")
    province = provinceLi.data("address")
    ...
    if li.hasClass("selected")
      ...
      return
    ## 删除已经展开过的节点，防止无限叠加
    $('.region-list.js-address-list[data-level="4"]').remove()
    li.addClass("selected").siblings().removeClass "selected"
    unless li.find(".city-list").length
      $.ajax
        type: "GET"
        ...
        success: (regions)=>
          ...
          data = _.map regions, (i)=>
            ...
            #    map = new RegExp("(.*)#{j}")
            #    i.name = i.name.match(map)[1]
            i.checked = true if (_.contains address, i.id.toString()) or (provinceLi.find(".js-select-province-item").prop "checked") or (li.find(".js-select-city-item").prop "checked")
            i
          count = (_.filter(data, (i)-> if i.checked then i)).length
          li.find(".js-city-count").text("(#{count})") if count
  ...
  checkProvince: (evt)=>
    ...
    provinceItem = $target.closest("li")
    id = $target.val()
  ...
  checkCity: (evt)=>
    ...
    checkedCities = cityArea.find(".js-select-city-item").filter(()-> @indeterminate or @checked)
  ...
  checkProvinceStatus: (province)=>
    ...
    cityIndeterminateCount = _.filter(cityCount, (i) => $(i).prop("indeterminate") is true)
  ...
  showAddress: (area)->
    ...
    showData = _.flatten _.map $(".js-select-province-item").filter(()-> @indeterminate or @checked), (i) ->
      item = $(i).closest("li")
  ...
  checkArea: (evt) =>
    ...
    $template = $self.closest(".js-template-item")
    regionIds = $self.data("area").split(",")
    $provinceItem = $template.find(".js-select-province-item").filter((i, d) => _.contains(regionIds, $(d).val()))
   ...
   * 获取区域信息
  ###
  organizeAreas: ($container)=>
    template = $(".js-template-item", $container)
    province = template.find(".js-select-province-item").filter(()->@checked or @indeterminate)
    _.map province, (i) =>
