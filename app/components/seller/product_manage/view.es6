...
const batchOnResult = Handlebars.templates["seller/product_manage/templates/batchOnResult"]
...
class ProductManage extends OriginShelfItem {
  ...
  deleteTheItem (evt) {
    let itemId = $(evt.currentTarget).closest("tr").data("id");
    let ifsupperPro = $(evt.currentTarget).closest("tr").data("ifpro") == true;
    $(".js-item-delete").attr("id", itemId);
    $(document).on("confirm:delete-one", (event, data)=> this.itemsDelete(data,ifsupperPro))
  }
  itemsDelete(data,ifsupperPro){
    let _url = ifsupperPro ? '/api/seller/items/block/delete' : '/api/seller/items/delete';
    $.ajax({
      url: _url,
      type: "POST",
      data: {
  ...
  batchItemOnshelfExamin(itemIds, auditComment) {
    ...
    $.ajax({
      ...
      let modal = new Modal(batchOnResult(result))
      modal.modal.find('.js-confirm').on('click', () => {
        modal.close()
        window.location.reload()
      })
      modal.show()
