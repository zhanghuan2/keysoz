...
class ProductManageMarket extends OriginShelfItem {
  ...
  offTheItem (evt) {
    ...
    $(document).off('confirm:off-one').on('confirm:off-one', (event, data) => this.undershelf(itemId))
  }
  // 单个商品下架请求
  undershelf(itemId){
    $.ajax({
      url: '/api/seller/items/mfacture/underShelf',
      type: 'POST',
      data: {
        'itemId': itemId
      },
  ...
  onTheItem (evt) {
    ...
    $(document).off('confirm:on-one').on('confirm:on-one', function (event, data) {
     /* let listExamine = $(listExamineTemplate({
      ...
      vm.itemOnshelfExamin(data, "",signDistrictCode)
    })
  }
  ...
  itemsDelete(data){
    let _url = '/api/seller/items/mfacture/delete';
    $.ajax({
      url: _url,
      type: "POST",
      data: {
  ...
  itemOnshelfExamin (itemId, auditComment,signDistrictCode) {
    ...
    $('body').spin('medium')
    $.ajax({
      url: '/api/seller/items/mfacture/applyOnshelf',
      type: 'POST',
      data: {
        'itemId': itemId,
