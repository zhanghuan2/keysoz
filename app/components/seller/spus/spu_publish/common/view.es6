...
const attributeTemplate = Handlebars.templates["seller/spus/spu_publish/common/templates/attribute_template"],
      skuAttibuteTemplate = Handlebars.templates["seller/spus/spu_publish/common/templates/sku_attribute"],
      skuTableTemplate = Handlebars.partials["seller/spus/spu_publish/common/all_templates/_sku_table"],
      itemImageTemplate = Handlebars.templates["seller/spus/spu_publish/common/templates/image"],
      skuValueTemplate = Handlebars.partials["seller/spus/spu_publish/common/templates/_sku_value"],
      attributeImageTemplate = Handlebars.partials["seller/spus/spu_publish/common/all_templates/_attribute_image"],
      skuInfoTemplate = Handlebars.partials["seller/spus/spu_publish/common/all_templates/_sku_info"],
      attributeDropDownTemplate = Handlebars.partials["seller/spus/spu_publish/common/all_templates/_attribte_dropdown"],
      attributeInputTemplate = Handlebars.partials["seller/spus/spu_publish/common/all_templates/_attribute_input"],
      attrErrorTipTemplate = Handlebars.templates["seller/spus/spu_publish/common/templates/attribute_error_tip"]
...
export default class SpuPublish {
  ...
  bindEvent () {
    ...
    $select_brand.select2({
      ...
      placeholder: "请选择",
      ajax: {
        url: "/api/brands/seller",
        dataType: "json",
        delay: 500,
        data: {"cid":$.query.get("cid")},
        error: function (e) {
          console.log(e);
        },
    ...
    $(".xhbox input").on("focus",function(){
      ...
      let selectBrandName = $select_brand.data("name")?$select_brand.data("name"):"";
      $("#input-name").val(selectBrandName+$(this).val())
      setTimeout(function(){
  ...
  getOriginCanSkuKey (attrs) {
    return _.without(_.flatten(_.map(attrs, (i) => {
      if (i.attributeRule && i.attributeRule.attrMetasForK) {
        return i.attributeRule.attrMetasForK.PROPERTY_TYPE === "SKU_CANDIDATE" ? i.attrKey : 0
  ...
  organizeItemBaseInfo (form) {
    ...
    item.brandId = $("#js-item-select-brand").val()
    item.brandName = $("#js-item-select-brand").find("option:selected").text()
