...
class ProductManageMarket extends OriginShelfItem {
  ...
  bindEvent () {
    ...
    $(".item-manage-table").find(".js-item-delete ").on("click",function(evt){
      let itemId = $(evt.currentTarget).closest("tr").data("id");
      $(".js-item-delete").attr("id", itemId);
      $(document).on("confirm:delete-one", (event, data)=> self.itemsDelete(data))
    })
  }
  itemsDelete(data){
    $.ajax({
      url: '/api/seller/items/block/delete',
      type: "POST",
      data: {
        "itemId": data
      },
  ...
  undershelf(itemId,signDistrictCode){
    $.ajax({
      url: '/api/seller/items/block/underShelf',
      type: 'POST',
      data: {
        'itemId': itemId,
        'districtCode':signDistrictCode
      },
      success: () => {
        window.location.reload();
      }
  ...
  handleCancelAudit (evt) {
    ...
    $('.js-cancel-submit').on('click', function () {
      ...
      $('body').spin('medium')
      $.ajax({
        url: '/api/seller/items/block/applyRevoke',
        type: 'POST',
        data: {
          'itemId': itemId,
  ...
  unfrozenItem (itemId, comment,signDistrictCode) {
    let api = '/api/seller/items/block/applyUnfreeze'
    $('body').spin('medium')
    $.ajax({
      url: api,
      type: 'POST',
  ...
  itemOnshelfExamin (itemId, auditComment,signDistrictCode) {
    ...
    $('body').spin('medium')
    $.ajax({
      url: '/api/seller/items/block/applyOnshelf',
      type: 'POST',
      data: {
        'itemId': itemId,
        'comment': auditComment,
        'applyTrigger': 'SUBMIT_FIRST_AUDIT',
        'districtCode':signDistrictCode
      },
      success: () => {
        window.location.reload()
  ...
  renderItemDecreaseRate(){
    $('.item-decrease-rate').each((i, el)=>{
      let min = $(el).data('min')||0,
        max = $(el).data('max')||0;
      min = parseInt(min);
      max = parseInt(max);
      if(min==0 && max==0){
        $(el).text(`${(min / 100).toFixed(2)}%`);
      }else if(min==0 && max!=0){
        $(el).text(`0%-${(max / 100).toFixed(2)}%`);
      }else if(min!=0 && max==0){
        $(el).text(`${(min / 100).toFixed(2)}%`);
      }else{
        if(min && max){
          if(min == max){
            ...
            $(el).text(`${(min / 100).toFixed(2)}%-${(max / 100).toFixed(2)}%`);
          }
        }
        else if(min) {
          $(el).text(`${(min / 100).toFixed(2)}%`);
