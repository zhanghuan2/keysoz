...
// 渲染商品评价,hasAppend参数代表是否有追评
	renderComment (pageNo = 1, pageSize = 20) {
		$(".jsEvaluateList").spin("large")
		let itemId = $ (".tab-navs").data ("item"), self = this, hasAppend = self.$hasAppend, hasPictrue = self.$hasPictrue
		$.ajax ({
			url: "/api/credit/evaluate/getMoreItemEvaluateDetail",
			type: 'GET',
			data: {itemId, pageNo, pageSize, hasAppend,hasPictrue},
		}).done (function (resp) {
			$(".jsEvaluateList").spin(false)
			...
			resp.hasNoEvalute = self.$hasNoEvalute
...
// 商品评价tab切换
	showEvaluate (evt) {
		...
		if (self.data ("filter") == "append") {
			this.$hasAppend = true
			this.$hasNoEvalute = "追评"
		} else {
			this.$hasAppend = false
		}

		if(self.data ("filter") == "hasImg"){
			this.$hasPictrue = true
			this.$hasNoEvalute = "图片评价"
		}else{
			this.$hasPictrue = ""
		}
