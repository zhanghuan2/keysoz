...
class ShoppingCart{
  constructor($){
    ...
    //this.vaccineCartSubmit = $(".vaccine .js-cart-submit");
  ...
  deleteInvalidItems(evt){
    ...
    $.ajax({
      type: "DELETE",
      url: "/api/zcy/block/cart/out-date",
  ...
  submitOrder(evt){
    ...
    $.ajax({
        type:"POST",
        url:"/api/zcy/carts/purchaser/submit?cartClass=3",
        data: skus,
        dataType: "text",
        ...
        success:(data)=>{
          ...
          window.location.href = frontUrl + "/buyer/purchase-process?purchaseId="+data;
        }
    }).fail(()=>{
  ...
  submitCart(evt,needSubmitRequ){
    ...
    let orgId = $('.wangchao .supperPro-cart-foot-operate').data("orgid");
    $.ajax({
        type:"POST",
        ...
        success:(data)=>{
          ...
          // 生成 需求单
            ...
            let frontUrl = $('input[name="frontUrl"]').val();
            window.location.href = frontUrl + "/buyer/requisition-process?reqId=" + data+"&orgId="+orgId;
          }
        },
        error:(data)=>{
  ...
  xieyiSubmitCart(evt,needSubmitRequ){
    ...
    let orgId = $('.xieyi .supperPro-cart-foot-operate').data("orgid");
    $.ajax({
        type:"post",
  ...
  vaccineSubmitCart(evt,needSubmitRequ){
    ...
    let orgId = $('.vaccine .supperPro-cart-foot-operate').data("orgid");
    $.ajax({
      type:"post",
      url:"/lasvegas/finance/api/protocol/hall/saveHallRequirement",//早晚要改，参见app/components/goodsHall/spu_tab2/extend.coffee
      data:{jsonStr : JSON.stringify(arrs)},
  ...
  submitRequ(evt,reqId){
    $.ajax({
      ...
      success:(data)=>{
        //console.log("success:"+data);
        let frontUrl = $('input[name="frontUrl"]').val();
        window.location.href = frontUrl + "/buyer/select-requisitions";
      },
      error:(data)=>{
        //console.log("error: "+data.responseText);
        let resultJson;
        try{
          ...
          if(resultJson.type != undefined){
            ...
            new Modal({
              ...
              let frontUrl = $('input[name="frontUrl"]').val();
              window.location.href= frontUrl + "/buyer/requisition-process?reqId=" + reqId;
            });
          }
        }catch(e){
          new Modal({
  ...
  initItemStatus(){
    ...
    item_trs.each( (i, d) => {
      ...
      if(isNaN(stock) || isNaN(quantity)){ return true; };
    ...
    //下浮率展示
    $('.js-decrease-rate').each((i, el)=>{
      let rate = $(el).data('rate');
      if(rate < 0) {
        $(el).text('--');
      }
      else{
        $(el).text((rate/100).toFixed(2) + '%');
      }
    })
  }
  ...
  changeNumber(el,mParent) {
    ...
    sendObj.classification = 3;
    //sendObj.classification = mParent.hasClass("wangchao") ? 0 : 1;
    $.ajax({
      url: "/api/zcy/block/cart/change",
      type: "PUT",
      data: sendObj,
      success: (data) =>{}
    });
  ...
  deleteTheItem(evt){
    let id = $(evt.target).closest(".item-tr").data("id"),
      thirdId = [],
      thi = $(evt.target).closest(".item-tr").find("[data-thirdid]").attr("data-thirdid");
    if(thi.length > 0){
      thirdId.push(thi);
    }
    this.deleteItem(id, thirdId);
  }
  ...
  deleteBatchItem(evt){
    ...
    this.deleteItem(ids.join(","), thirdId);
  }

  //发送删除商品请求
  deleteItem(ids, thirdId){
    var obj = {
      skuIds : ids,
      thirdIds : thirdId.join(",")
    };
    obj.classification = 3;
    $.ajax({
      url: "/api/zcy/block/cart/batchDelete",
      type: "POST",
      data: obj,
      success: ()=>{
  ...
  runScroll(evt){
    let footDiv = $(".supperPro-cart-foot");
    let originY = $(".supperPro-cart-body").offset().top + $(".supperPro-cart-body").height();
    ...
    if (!footDiv.hasClass("float-foot") && (originY - $(".float-hidden").offset().top) > ($(window).height() - footDiv.height())){
      ...
      footDiv.css("left", ($(".supperPro-cart-body").offset().left - 1));
    }
  }
  ...
  createOrder (evt) {
    ...
    let frontUrl = $('input[name="frontUrl"]').val();
    window.location.href = frontUrl + `/buyer/create-order?fromCart=1&orderType=${orderType}`;
  }
