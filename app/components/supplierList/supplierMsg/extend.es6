...
class supplierMsg {
  ...
  init(){
    ...
    $.each($save,function(i,v){
      if($(this).val()){
	      ...
	      if(tar-avg>0){
		      $(".component-supplierMsg .supplier-avg-score-box").find("."+name).find("i").addClass("icon-jiantouxiangshang avg-up")
		      ...
		      $(".component-supplierMsg .supplier-avg-score-box").find("."+name).find("i").addClass("icon-jiantouxiangxia avg-down")
  ...
  bindEvent(){
    ...
    $(".content-qq a").on("click",(evt) => this.contactFun(evt));
  ...
  contactFun(evt){
    let $qqlink = $(".component-supplierMsg .content-qq a");
